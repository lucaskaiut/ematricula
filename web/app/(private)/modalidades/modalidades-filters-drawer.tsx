...
import type { ModalitiesListUrlState } from './modalidades-url-state';

import 'react-day-picker/style.css';

export type ModalitiesFiltersDrawerProps = {
  open: boolean;
...
function DateRangePopoverField({
  ...
  useEffect(() => {
    ...
    const onPointerDown = (e: MouseEvent) => {
      ...
      if (root?.closest('[data-modalities-date-popover]')) return;
      setPickerOpen(false);
    };
...
export function ModalitiesFiltersDrawer({
  open,
  onOpenChange,
  committed,
  onApply,
}: ModalitiesFiltersDrawerProps) {
  const titleId = useId();
  ...
  const handleApply = useCallback(() => {
    ...
    onOpenChange(false);
  }, [created, description, onApply, onOpenChange, updated]);
    ...
    <div className="fixed inset-0 z-50" role="presentation">
      ...
      >
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-6 overflow-y-auto overflow-x-visible overscroll-contain px-4 py-5">
          <div>
            <label htmlFor="filter-description" className={labelClass}>
              Descrição
            </label>
            <input
