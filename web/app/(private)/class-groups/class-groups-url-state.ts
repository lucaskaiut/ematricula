import {
  URL_ENCODED_STATE_KEY,
  decodeUrlStatePayload,
  encodeUrlStatePayload,
} from '@/lib/url-state';

export type ClassGroupsListUrlState = {
  v: 1;
  page: number;
  sort: Array<{ key: string; direction: 'asc' | 'desc' }>;
  name: string;
  modalityId: number | null;
  teacherPersonId: number | null;
};

function toPositiveInt(value: string | null, fallback: number) {
  const n = Number(value);
  if (!Number.isFinite(n)) return fallback;
  const i = Math.trunc(n);
  return i > 0 ? i : fallback;
}

function getSortFromSearchParams(
  searchParams: URLSearchParams,
): Array<{ key: string; direction: 'asc' | 'desc' }> {
  const result: Array<{ key: string; direction: 'asc' | 'desc' }> = [];
  for (const [key, value] of searchParams.entries()) {
    if (!key.startsWith('orderBy[') || !key.endsWith(']')) continue;
    const field = key.slice('orderBy['.length, -1);
    if (!field) continue;
    const dir = value === 'desc' ? 'desc' : value === 'asc' ? 'asc' : null;
    if (!dir) continue;
    result.push({ key: field, direction: dir });
  }
  return result;
}

const LIKE = 'like';

function stripLikeWildcards(value: string) {
  if (value.length >= 2 && value.startsWith('%') && value.endsWith('%')) {
    return value.slice(1, -1);
  }
  return value;
}

function getLikeFilterDisplay(searchParams: URLSearchParams, field: string) {
  const opKey = `filters[${field}][0]`;
  const valKey = `filters[${field}][1]`;
  const op = searchParams.get(opKey);
  const raw = searchParams.get(valKey);
  if (op === LIKE && raw !== null && raw !== '') {
    return stripLikeWildcards(raw);
  }
  const legacy = searchParams.get(`filters[${field}]`);
  return (legacy ?? '').trim();
}

function parseOptionalId(searchParams: URLSearchParams, field: string): number | null {
  const raw = searchParams.get(`filters[${field}]`);
  if (raw === null || raw === '') return null;
  const n = Number(raw);
  if (!Number.isFinite(n) || n <= 0) return null;
  return Math.trunc(n);
}

export function defaultClassGroupsListUrlState(): ClassGroupsListUrlState {
  return {
    v: 1,
    page: 1,
    sort: [],
    name: '',
    modalityId: null,
    teacherPersonId: null,
  };
}

export function normalizeClassGroupsListUrlState(
  input: ClassGroupsListUrlState,
): ClassGroupsListUrlState {
  const sort = input.sort.filter(
    (s): s is { key: string; direction: 'asc' | 'desc' } =>
      !!s &&
      typeof s.key === 'string' &&
      s.key.length > 0 &&
      (s.direction === 'asc' || s.direction === 'desc'),
  );
  return {
    v: 1,
    page: Math.max(1, Math.trunc(Number(input.page)) || 1),
    sort,
    name: typeof input.name === 'string' ? input.name : '',
    modalityId:
      typeof input.modalityId === 'number' && input.modalityId > 0 ? input.modalityId : null,
    teacherPersonId:
      typeof input.teacherPersonId === 'number' && input.teacherPersonId > 0
        ? input.teacherPersonId
        : null,
  };
}

export function parseLegacyClassGroupsListUrlState(
  searchParams: URLSearchParams,
): ClassGroupsListUrlState {
  return normalizeClassGroupsListUrlState({
    v: 1,
    page: toPositiveInt(searchParams.get('page'), 1),
    sort: getSortFromSearchParams(searchParams),
    name: getLikeFilterDisplay(searchParams, 'name'),
    modalityId: parseOptionalId(searchParams, 'modality_id'),
    teacherPersonId: parseOptionalId(searchParams, 'teacher_person_id'),
  });
}

function isDecodedClassGroupsState(value: unknown): value is Record<string, unknown> {
  if (!value || typeof value !== 'object') return false;
  const o = value as Record<string, unknown>;
  if (o.v !== 1) return false;
  if (!Array.isArray(o.sort)) return false;
  if (typeof o.name !== 'string') return false;
  return true;
}

function decodedToClassGroupsListState(o: Record<string, unknown>): ClassGroupsListUrlState {
  const page = typeof o.page === 'number' ? o.page : Number(o.page);
  const modalityId =
    typeof o.modalityId === 'number' && o.modalityId > 0 ? o.modalityId : null;
  const teacherPersonId =
    typeof o.teacherPersonId === 'number' && o.teacherPersonId > 0
      ? o.teacherPersonId
      : null;
  return normalizeClassGroupsListUrlState({
    v: 1,
    page: Number.isFinite(page) ? page : 1,
    sort: o.sort as ClassGroupsListUrlState['sort'],
    name: o.name as string,
    modalityId,
    teacherPersonId,
  });
}

export function parseClassGroupsListUrlState(
  searchParams: URLSearchParams,
): ClassGroupsListUrlState {
  const packed = searchParams.get(URL_ENCODED_STATE_KEY);
  if (packed) {
    const decoded = decodeUrlStatePayload<unknown>(packed);
    if (isDecodedClassGroupsState(decoded)) {
      return decodedToClassGroupsListState(decoded);
    }
  }
  return parseLegacyClassGroupsListUrlState(searchParams);
}

export function hasClassGroupsListLegacyFlatParams(searchParams: URLSearchParams): boolean {
  if (searchParams.has('page')) return true;
  for (const key of searchParams.keys()) {
    if (key.startsWith('orderBy[') || key.startsWith('filters[')) return true;
  }
  return false;
}

export function encodeClassGroupsListUrlState(state: ClassGroupsListUrlState): string | null {
  const normalized = normalizeClassGroupsListUrlState(state);
  const baseline = defaultClassGroupsListUrlState();
  if (
    normalized.page === baseline.page &&
    normalized.sort.length === 0 &&
    normalized.name.trim() === '' &&
    normalized.modalityId === null &&
    normalized.teacherPersonId === null
  ) {
    return null;
  }
  return encodeUrlStatePayload(normalized);
}

export function buildClassGroupsListPath(
  pathname: string,
  state: ClassGroupsListUrlState,
  extraSearchParams?: URLSearchParams,
): string {
  const encoded = encodeClassGroupsListUrlState(state);
  const params = new URLSearchParams();
  if (extraSearchParams) {
    extraSearchParams.forEach((value, key) => {
      if (key === URL_ENCODED_STATE_KEY) return;
      params.append(key, value);
    });
  }
  if (encoded) params.set(URL_ENCODED_STATE_KEY, encoded);
  const qs = params.toString();
  return qs ? `${pathname}?${qs}` : pathname;
}

export function classGroupsListStateToApiParams(state: ClassGroupsListUrlState): URLSearchParams {
  const normalized = normalizeClassGroupsListUrlState(state);
  const p = new URLSearchParams();
  if (normalized.page > 1) p.set('page', String(normalized.page));
  for (const s of normalized.sort) {
    p.append(`orderBy[${s.key}]`, s.direction);
  }
  const name = normalized.name.trim();
  if (name) {
    p.set('filters[name][0]', LIKE);
    p.set('filters[name][1]', `%${name}%`);
  }
  if (normalized.modalityId !== null) {
    p.set('filters[modality_id]', String(normalized.modalityId));
  }
  if (normalized.teacherPersonId !== null) {
    p.set('filters[teacher_person_id]', String(normalized.teacherPersonId));
  }
  return p;
}

export function removeUpdatedParam(searchParams: URLSearchParams): URLSearchParams {
  const next = new URLSearchParams(searchParams.toString());
  next.delete('updated');
  return next;
}
