...
{
    protected $signature = 'invoices:send-due-reminders {--date= : Data de referência Y-m-d (padrão: hoje)}';

    protected $description = 'Envia lembrete de pagamento por e-mail nas faturas pendentes com vencimento na data informada';
            ...
            Invoice::query()
                ...
                ->chunkById(100, function ($invoices) use ($hub, $paymentService, $today, $invoicePaymentMethodResolver): void {
                    foreach ($invoices as $invoice) {
                        ...
                        $cacheKey = 'invoice:due-reminder:'.$invoice->company_id.':'.$invoice->id.':'.$today->toDateString();
